...
describe('RentalService', () => {
    ...
    let moviesDTO = [
        new MovieRentalDTO(1),
        new MovieRentalDTO(2),
    ]
    ...
    describe('createRental', () => {
        it('duplicate movies: throw BadRequestException', async () => {
            await expect(rentalService.createRental(rentalDuplicateMovies))
                .rejects
                .toThrow('movieId: 1, has duplicated in requested movies');
        });
    ...
    describe('devolveRental', () => {
        it('undefined rental: throw BadRequestException', async () => {
            await expect(rentalService.devolveRental(new DevolveRentalDTO(0)))
                .rejects
                .toThrow('rental does not exist or has already been returned');
        });
        it('shoud be return a rental devolved', async () => {
            const devolveRentalReturn = await rentalService.devolveRental(new DevolveRentalDTO(1));
