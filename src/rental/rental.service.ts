...
export class RentalService {
  ...
  async findOneById(rentalId: number): Promise<Rental> {
    return this.rentalRepository.findOne(rentalId, {
      relations: ['movies', 'movies.movie'],
    });
  }
  ...
  private validateMovieArray(movies: MovieRentalDTO[]) {
    ...
    if (duplicateMovies.length > 0) {
      throw new BadRequestException(
        `movieId: ${duplicateMovies[0].movieId}, has duplicated in requested movies`,
      );
    }
  }

  async devolveRental(devolveRentalDTO: DevolveRentalDTO): Promise<Rental> {
    const rental = await this.rentalRepository.findOne(devolveRentalDTO.id, {
      where: { returnDate: null },
      relations: ['movies', 'movies.movie'],
    });
    if (rental === undefined) {
      throw new BadRequestException(
        'rental does not exist or has already been returned',
      );
    }
  ...
  async expiredRentals(): Promise<any[]> {
    const rentalsObject = await this.rentalRepository.find({
      where: { returnDate: null },
      relations: ['movies', 'movies.movie'],
    });
    ...
    for (const [idx, rental] of rentals.entries()) {
      let daysOverdue = this.calculateDaysOverdue(
