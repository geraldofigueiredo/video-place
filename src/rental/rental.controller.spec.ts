...
describe('RentalController', () => {
    ...
    describe('rentMovies', () => {
        ...
        it('should throw a bad request exception: movies arrays not defined', async () => {
            await expect(rentalController.rentMovies(rentalDtoMoviesUndefined))
                .rejects
                .toEqual(new BadRequestException('movies array must be defined'));
        })

        it('should throw a bad request exception: movies arrays must be not empty', async () => {
            await expect(rentalController.rentMovies(rentalDtoMoviesEmpty))
                .rejects
                .toEqual(new BadRequestException('movies array must be not empty'));
        })
    ...
    describe('devolveRental', () => {
        it('should return the devolved rental', async () => {
            const devolvedRental = await rentalController.devolveRental(new DevolveRentalDTO)
